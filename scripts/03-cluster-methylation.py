...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Filtered combined H5AD generated by 02-matrix-qc.py.",
    )
    parser.add_argument(
        "--n-components",
        type=int,
...
def cluster_colors(cluster_count: int) -> list[tuple[float, float, float, float]]:
    ...
    colors.extend(
        palette(index / max(1, extra_count - 1))
...
def main() -> None:
    ...
    cluster_categories = [f"D{index}" for index in range(cluster_count)]
    adata.obs["cluster"] = pd.Categorical(
        [f"D{item}" for item in clusters],
        categories=cluster_categories,
        ordered=True,
    )
    adata.uns["cluster_colors"] = np.asarray(
        [
